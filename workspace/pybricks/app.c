...
mp_obj_t execute_from_file(const char *file) {
    ...
    if (nlr_push(&nlr) == 0) {
        ...
        mp_obj_t module_fun = mp_compile(&pt, qstr_from_str(file), MP_EMIT_OPT_NONE, false);
...
static void shutdown(intptr_t unused) {
    if (ev3_sdcard_opendir(SD_RES_FOLDER) >= E_OK) {
        // ext_ker();
        try_acquire_mmcsd();
    }
    else {
        show_message_box("Warning", "Please eject the USB!", true);
        return;
    }
}

static const CliMenuEntry entry_tab[] = {
    { .key = '5', .title = "Scripts", .handler = select_script },
    { .key = '6', .title = "Shutdown", .handler = shutdown },
};
